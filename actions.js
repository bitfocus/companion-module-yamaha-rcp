module.exports = {
	...
	createAction: (instance, rcpCmd) => {
		...
		switch (rcpCmd.Type) {
			case 'integer':
				// for toggles and relatives, make sure the current value is stored in dataStore[]
	...
	parseCmd: async (instance, prefix, rcpCmd, opt) => {
		...
		//      console.log(`value of ${opt.X} = `, optX)
		...
		switch (rcpCommand.Type) {
			...
			case 'string':
				cmdName = `${prefix} ${cmdName}`
				optVal = prefix == 'set' ? `"${await instance.parseVariablesInString(opt.Val)}"` : '' // parse vars and put quotes around the string
